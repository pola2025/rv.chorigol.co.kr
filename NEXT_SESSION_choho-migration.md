...
🔴 **다음 세션 0순위 — 첫 실발송을 확인해라 (2026-07-18 10:00 KST 퇴실)**
   ...
   ── 2026-07-17 밤 재확인 (다음 세션이 헷갈리지 않게) ────────────────
   · **여전히 유효하다.** D1 `notification_log` 의 7/17 04:00Z(=13:00 KST) checkIn **8건은
     Vercel 크론이 보낸 게 아니다** — CF 의 마지막 발송을 `backfill-live-smsstatus.mjs` 로
     Firestore 에서 옮겨온 기록이다. 이 8건을 보고 "크론 검증됐다"고 오판하기 쉽다. (실제로 했다)
   · **발송 코드(SENS) 자체는 검증됐다** — 7/17 21:47·21:48 예약확정 문자+텔레그램 4건이
     `lib/reservation-notify.js` 로 **실발송 success**. 예약 생성 12:46:59.799Z → 알림 12:47:01.375Z(1.6초).
     CF 함수는 0개이므로 이건 Vercel 이 보낸 것이다.
   · 따라서 **미검증으로 남은 건 오직 "크론 스케줄이 제때 깨어나는가"** 하나다.
   · 내일 10:00 퇴실 대상 **8건** 중 `단체(Forest 패밀리)` 는 수신번호가 `"0"` 이라
     **발송 실패가 정상**이다(감사 스크립트도 잡는다. CF 도 같았다). 나머지 7건이 나가야 한다.
     13:00 입실은 3건(김혜진·오재연·최현지) 전부 실번호다.
   ─────────────────────────────────────────────────
