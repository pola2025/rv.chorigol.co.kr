"use client";
// 예약 캘린더 (클라이언트) — 월 그리드 + 날짜별 상세 + 신규예약 생성 + 예약 편집.
...
export default function CalendarClient({ ym, reservations, rooms }) {
  ...
  return (
    <main
      style={{ maxWidth: 1100, margin: "0 auto", padding: "1.5rem 1.25rem" }}
    >
      <div
        style={{
          display: "flex",
      ...
      >
        ...
        >
          <a href={`/calendar?month=${shiftMonth(ym, -1)}`} style={navBtn}>
            ◀
          </a>
          ...
          >
            {WEEKDAYS.map((w, i) => (
              <div
                ...
                style={{
                  ...dowCell,
                  color: i === 0 ? "#a8422f" : i === 6 ? "#2f6b4f" : "#6a7a71",
                }}
              >
                {w}
              </div>
            ...
            >
              {dayList.map((r) => (
                ...
                >
                  ...
                  >
                    {r.room_name} · {r.check_in}~{r.check_out} · {r.guests}명 ·{" "}
                    {r.phone}
                    {r.options.length > 0 &&
              ...
              {dayList.length === 0 && !showForm && (
                <div style={{ color: "#6a7a71", fontSize: ".85rem" }}>
                  예약 없음
                </div>
              )}
            </div>
...
function BookingForm({ date, rooms, onDone }) {
  ...
  async function submit() {
    setErr("");
    if (!f.customer_name || !f.phone)
      return setErr("고객명·연락처는 필수입니다");
    setBusy(true);
    try {
      const res = await fetch("/api/reservations", {
        ...
        body: JSON.stringify({
          ...
          base_price: Number(f.total_price),
        }),
      });
  ...
  return (
    ...
    >
          ...
          <select
            ...
            onChange={(e) => {
              ...
              setF({
                ...
                total_price: room?.base_price || f.total_price,
              });
            }}
          >
            {rooms.map((r) => (
      ...
      {err && (
        <div
          style={{ color: "#a8422f", fontSize: ".78rem", marginTop: ".5rem" }}
        >
          {err}
        </div>
      )}
      <div style={{ display: "flex", gap: ".5rem", marginTop: ".75rem" }}>
        ...
        <span
          style={{ fontSize: ".72rem", color: "#6a7a71", alignSelf: "center" }}
        >
