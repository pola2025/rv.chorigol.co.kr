...
async function sendTelegramNotification(message, businessType = null) {
  try {
    ...
    const chatId = businessType
      ? TELEGRAM_CHAT_IDS[businessType]
...
const removeEmojis = (text) => {
  ...
  const emojiPattern =
    /[\u{1F600}-\u{1F64F}]|[\u{1F300}-\u{1F5FF}]|[\u{1F680}-\u{1F6FF}]|[\u{1F700}-\u{1F77F}]|[\u{1F780}-\u{1F7FF}]|[\u{1F800}-\u{1F8FF}]|[\u{2600}-\u{26FF}]|[\u{2700}-\u{27BF}]|[\u{1F900}-\u{1F9FF}]|[\u{1FA00}-\u{1FA6F}]|[\u{1FA70}-\u{1FAFF}]|[\uD800-\uDBFF][\uDC00-\uDFFF]/gu;
...
async function getTemplate(reservation, templateType) {
  try {
    ...
    if (docRef.exists) {
      ...
      if (
        data.roomSettings &&
        data.roomSettings[reservation.roomName]?.templates?.[templateType]
      ) {
        const roomTemplate =
          data.roomSettings[reservation.roomName].templates[templateType];
        console.log(
          `✅ [${businessType}] ${reservation.roomName} 객실별 ${templateType} 템플릿 사용`,
        );
        return typeof roomTemplate === "object"
      ...
      // 3. 기본 템플릿 생성
      console.log(
        `⚠️ [${businessType}] ${templateType} 템플릿 없음 - 기본 템플릿 사용`,
      );
...
function replaceTemplateVariables(template, reservation) {
  ...
  message = message.replace(
    /{인원}/g,
    reservation.guests || reservation.guestCount || 2,
  );
  message = message.replace(
    /{금액}/g,
  ...
  // 주소 추가
  const address = "경기도 파주시 법원읍 초리골길 134";
...
export const autoSendSMSScheduler = onSchedule(
  ...
  async (context) => {
    ...
    if (hour === 13) {
      ...
      console.log(
        `현재 시간(${hour}시)은 SMS 발송 시간(10시, 13시)이 아닙니다.`,
      );
      return;
    }
    ...
    try {
      ...
      for (const doc of reservationsSnapshot.docs) {
        ...
        if (reservation.smsStatus?.[`${smsType}Sent`]) {
          console.log(
            `✅ 이미 발송됨: ${reservation.customerName} (${reservation.roomName})`,
          );
          continue;
        }
        ...
        if (!autoSendEnabled) {
          console.log(
            `⏸️ 자동발송 비활성화: ${reservation.customerName} (${reservation.roomName})`,
          );
          continue;
        }
        ...
        try {
          ...
          if (result.success) {
            ...
            console.log(
              `✅ 발송 성공: ${reservation.customerName} (${reservation.roomName})`,
            );
            sendResults.push({
              roomName: reservation.roomName,
              customerName: reservation.customerName,
            ...
            console.error(
              `❌ 발송 실패: ${reservation.customerName} - ${result.error}`,
            );
            sendResults.push({
              roomName: reservation.roomName,
              customerName: reservation.customerName,
      ...
      if (sendResults.length > 0) {
        ...
        // 업체별로 결과 분리
        const chohoResults = sendResults.filter(
          (r) => getBusinessType(r.roomName) === "choho",
        );
        const shelterResults = sendResults.filter(
          (r) => getBusinessType(r.roomName) === "shelter",
        );
