...
// ─────────────────────────────────────────────
// 객실 (rooms) — 컴포넌트는 **한글 필드**로 폼을 만든다 (legacy-shape.toRoom 의 역방향)
// ─────────────────────────────────────────────
const ROOM_MAP = {
  객실명: "name",
  기본요금: "base_price",
...
/** 무해 확인된 것만. 서버가 직접 찍거나 경로로 받는다 */
const ROOM_DROP = new Set(["id", "createdAt", "updatedAt", "created", "updated"]);

/** 객실 폼 → API 본문. 넘어온 키만 변환(부분 수정 지원) */
export function toRoomWriteBody(data = {}) {
  return mapWith(ROOM_MAP, ROOM_DROP, data, "room");
}

// ─────────────────────────────────────────────
// 옵션 (options) — 대부분 camelCase 그대로지만 JSON 컬럼 이름이 다르다
// ─────────────────────────────────────────────
const OPTION_MAP = {
  name: "name",
  type: "type",
  ...
  applicableRooms: "applicable_rooms", // **모드 문자열** (all|selected|individual|shared)
  selectedRooms: "selected_rooms",     // 실제 객실 목록 — applicableRooms 와 다른 필드다
  roomPrices: "room_prices",
  roomStocks: "room_stocks",
...
/**
 * `isDefault` 는 버린다 — 기본 옵션은 `options` 테이블이 아니라 `option_settings` 로 간다
 * (레거시도 isDefault 면 settings 문서에, 아니면 options 컬렉션에 저장했다).
 * `priceType` 도 option_settings 전용 필드다(extra_person.priceType='per_person').
 */
const OPTION_DROP = new Set([
  "id",
  "isDefault",
  "priceType",
  "createdAt",
...
/** 옵션 폼 → API 본문 */
export function toOptionWriteBody(data = {}) {
  return mapWith(OPTION_MAP, OPTION_DROP, data, "option");
}

/** 공통 변환 — 모르는 키는 경고 후 버린다 (조용한 유실이 이관 버그의 주범이었다) */
function mapWith(map, drop, data, label) {
  const out = {};
  const unknown = [];
  for (const [k, v] of Object.entries(data || {})) {
  ...
  if (unknown.length && process.env.NODE_ENV !== "production")
    console.warn(
      `[legacy-write-shape:${label}] 매핑 없는 필드 무시: ${unknown.join(", ")}`,
    );
  return out;
}
