...
// 기록은 notification-log.js 단일 통로로 한다 — 예전엔 여기서 직접 INSERT 하고
// `.catch(() => {})` 로 실패를 삼켰다(문자는 나갔는데 신호기가 회색인데 아무도 모름).
// 공통 모듈이 재시도 + 실패 시 인프라봇 알림까지 해준다. 호출부 시그니처는 그대로다.
import { logNotification as log } from "./notification-log.js";
import {
  formatNew,
  formatConfirmation,
  formatCancellation,
