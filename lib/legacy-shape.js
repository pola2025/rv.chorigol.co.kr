...
export async function loadSnapshot() {
  const [
    ...
    query(`SELECT id, stock, data FROM inventory_overrides`),
  ...
  return {
    ...
    //
    // ⚠️ **stock 컬럼이 단일 소스**다. 재고 가드(lib/inventory.js)가 stock 을 읽으므로
    //    화면이 data.available 을 읽으면 두 소스가 갈라진다 — 화면엔 여유가 보이는데
    //    예약이 거절되거나 그 반대. 쓰기(updateInventoryOverride)는 stock 을 갱신한다.
    //    data.available 폴백은 **stock 이 NULL 인 랜덤 id 3건 전용**이다. 그 행들은 레거시가
    //    `${date}_${room}` 로만 조회해 읽히지 않는 죽은 데이터라, 폴백이 있어야 스토어 맵이
    //    레거시와 완전히 같아진다(값이 사라지면 맵 키의 값만 undefined 로 달라짐).
    overrides: Object.fromEntries(
      overrides.results.map((o) => [o.id, o.stock ?? parseJson(o.data)?.available]),
    ),
    blockedDates: [],
