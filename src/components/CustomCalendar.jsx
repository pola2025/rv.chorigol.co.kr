...
const CustomCalendar = ({ 
  ...
  onDateClick = () => {},
  onBookingClick = () => {},
  selectedDate = null // 선택된 날짜 prop 추가
}) => {
  ...
  // 예약 클릭 핸들러 - 모바일에서는 날짜 선택으로 처리
  const handleBookingClick = (e, booking, dateStr) => {
    e.stopPropagation();
    
    if (isMobile) {
      // 모바일: 날짜 선택으로 처리 (모달 열지 않음)
      onDateClick(dateStr);
    } else {
      // PC: 기존대로 모달 열기
  ...
  const renderDateCell = (day) => {
    ...
    const isSelected = selectedDate === dateStr; // 선택된 날짜 확인
    ...
    return (
      <div
        key={day}
        className={`calendar-cell ${isPast ? 'past' : ''} ${isToday ? 'today' : ''} ${isWeekend ? 'weekend' : ''} ${isSelected ? 'selected' : ''} `}
        onClick={() => onDateClick(dateStr)} // 과거 날짜도 클릭 가능

        aria-selected={isSelected}
        data-date={dateStr} // 디버깅용
      >
        ...
        <div className="cell-content">
          {rooms.map(room => {
            ...
            const bookingCount = bookingsForRoom.length; // 예약 건수
                        ...
                        {options.map((option, idx) => {
                          if (option.price > 0) {
                            if (option.onsite) {
                              // 현장결제 항목
                              return (
                                <span key={idx} className="addon-info onsite">
